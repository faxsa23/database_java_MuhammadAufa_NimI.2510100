...
public class CKaryawan {
    ...
    public CKaryawan() {
        if (!c.openConnection()) {
            System.err.println("⚠️ Koneksi database tidak tersedia.");
        }
    }

    private Connection getDbConnection() throws SQLException {
        if (c == null || c.getConnection() == null) {
            throw new SQLException("Koneksi database belum terbuka atau gagal dibuat.");
        }
    ...
    public ResultSet getRecords() {
        ...
        try {
            Statement stmt = getDbConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
